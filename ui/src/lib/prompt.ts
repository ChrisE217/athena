import type { AthenaAttempt, AthenaError, AthenaStep, AthenaTest } from '../types';
...
function primaryError(attempt: AthenaAttempt): AthenaError | undefined {
  ...
  return findFailedStep(attempt.steps)?.error;
}
...
export function buildFixPrompt(test: AthenaTest, attempt?: AthenaAttempt): string {
  ...
  lines.push(
    ...
    `- screenshot: ${att.attachments.some((a) => a.kind === 'screenshot') ? 'yes' : 'no'}`,
    `- trace: ${att.attachments.some((a) => a.kind === 'trace') ? 'yes' : 'no'}`,
    `- video: ${att.attachments.some((a) => a.kind === 'video') ? 'yes' : 'no'}`,
    '',
    '# Ask',
...
export function truncateForUrl(prompt: string): { urlPrompt: string; truncated: boolean } {
  if (prompt.length <= URL_LIMIT) return { urlPrompt: prompt, truncated: false };
  return {
    urlPrompt:
      prompt.slice(0, URL_LIMIT) +
      '\n\n[Truncated for URL length — full prompt was copied to clipboard]',
    truncated: true,
  };
}
